...
m1 <- glmmTMB(AUC ~ training_set + (1 | fold), data = control_df, family = gaussian)
...
I then did the same analysis for Yoden's J and checked assumptions.
```{r, include = FALSE}
m1 <- glmmTMB(Accuracy ~ model_type * training_set + (1 | fold), data = df)
...
Again variance from folds were small. There were the same fixed effects as before. For Accuracy, One-class is significantly worse than the control in condition all and there are no other significant differences.
