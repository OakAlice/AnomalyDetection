...
def main():
    ...
    for FOLD, TRAINING_SET, MODEL_TYPE in product(FOLDS, TRAINING_SETS, MODEL_TYPES):
        ...
        if MODEL_TYPE == 'multi':
            for BEHAVIOUR_SET in BEHAVIOUR_SETS:
                threshold_list = THRESHOLD_OPTIONS if BEHAVIOUR_SET == 'Activity' else [False]

                for THRESHOLDING in threshold_list:
                    ...
                    # Run TestModelClosed only if THRESHOLDING is False
                    if not THRESHOLDING:
                        TestModelClosed.main(BASE_PATH, DATASET_NAME, TRAINING_SET, MODEL_TYPE, 
                                            BEHAVIOUR_SET, THRESHOLDING, FOLD)
        else:
            TestModelOpen.main(BASE_PATH, DATASET_NAME, TRAINING_SET, MODEL_TYPE, 
